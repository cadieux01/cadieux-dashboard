export default function KPICard({ title, value, subtitle, icon, trend, trendUp, color = 'indigo' }) {
  ...
  return (
    <div
      className={`dashboard-panel rounded-2xl p-4 ${theme.border}`}
    >
      <div className={`absolute -right-8 -top-10 h-20 w-20 rounded-full bg-gradient-to-br ${theme.glow} blur-3xl`} />
      <div className="relative flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold uppercase tracking-[0.18em] ${theme.badge}`}>
            {title}
          </div>
          <p className="mt-2 font-display text-[2rem] font-semibold leading-none tracking-[-0.04em] text-white">
            {value}
          </p>
          {subtitle && <p className="mt-1.5 text-[13px] text-slate-400">{subtitle}</p>}
          {trend !== undefined && (
            <div
              className={`mt-2 inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[13px] font-semibold ${
                trendUp ? 'bg-emerald-400/12 text-emerald-200' : 'bg-rose-400/12 text-rose-200'
              }`}
            >
              <svg className={`h-3.5 w-3.5 ${trendUp ? '' : 'rotate-180'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {icon && (
          <div className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl [&_svg]:h-5 [&_svg]:w-5 ${theme.icon}`}>
            {icon}
          </div>
        )}
      </div>
